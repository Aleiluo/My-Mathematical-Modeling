...
class Algorithms:
    ...
    def NSGA_main(self):
        ...
        for i in range(self.gene_size):
            # Adaptive operator rate
            self.pc = self.pc_max - ((self.pc_max - self.pc_min) / self.gene_size) * i
            self.pm = self.pm_max - ((self.pm_max - self.pm_min) / self.gene_size) * i
            ...
            fitness.append((self.Pop[0].fitness[0],self.Pop[0].fitness[1],self.Pop[0].fitness[2]))
            pc.append(self.pc)
            pm.append(self.pm)
            
        EP=fast_non_dominated_sort(self.Pop)[0]
        return EP,fitness,pc,pm