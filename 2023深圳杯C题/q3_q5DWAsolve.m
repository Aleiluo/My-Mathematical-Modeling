function res = q3_q5DWAsolve(droneA, droneB, dwaParam, map)
% 输入：两个无人机的所有参数，DWA参数，地图参数
% 输出：A、B无人机到达终点的耗时

% 本函数专为灵敏度分析设计，去除了较为激进的动态障碍物估计
...
for i = 1:dwaParam.maxIter
    ...
    if dwaParam.displayIter < 1e5
        ...
        % title(num2str(dis2goalB));
        % 绘图验证采样点是否计算正确
%         if ~isempty(sample_points_handle)
%                 delete(sample_points_handle);
%         end
%         sample_points_handle = plot(droneA.samplePoints(:,1), droneA.samplePoints(:,2), 'x',Color='green',markersize=2);
    ...
    if dis2goalA < ToMidGoalErr && droneA.curgoal == size(droneA.goal, 1)
        % 此刻要更改向前采样时间逐渐逼近终点
        disA = calDis2cut(droneB.curpos(1:2), droneA.curpos(1:2), map.R);
        if disA > ToMidGoalErr
    ...
    if dis2goalB < ToMidGoalErr && droneB.curgoal == size(droneB.goal, 1)
        % 此刻要更改向前采样时间逐渐逼近终点
        disB = calDis2cut(droneA.curpos(1:2), droneB.curpos(1:2), map.R);
        if disB > ToMidGoalErr
...
function [best_w, score] = eval_bestw(main_drone, other_drone, dwaParam, map)
    ...
    % 判断下一步位置是否会撞在动态障碍物上(三到五问，不估计对方无人机的采样点)
    bestSamplePoint = other_drone.curpos;
