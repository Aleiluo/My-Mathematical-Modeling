function q4init_data(lon_range, lat_range)
    %% 海洋深度信息
    global depth_data;

    % 读取数据
    lon_elevation = ncread("data/Caribbean_Depth.nc", "lon");
    lat_elevation = ncread("data/Caribbean_Depth.nc", "lat");
    ...
    depth_data.elevation = double(ncread("data/Caribbean_Depth.nc", "elevation"));
...
%     rho_data.rho = pagetranspose(rho_data.rho);
    lon_rho = double(ncread("data/Caribbean_CTD.nc",'lon'));
    lat_rho = double(ncread("data/Caribbean_CTD.nc",'lat'));
    ...
    rho_data.rho = [];
    % 深度
    depth_salt = ncread("data/Caribbean_CTD.nc",'z');
    % 盐度
    salinity_salt = ncread("data/Caribbean_CTD.nc",'Salinity');
    % 每一个经纬度测了多少个盐度
    z_row_size = ncread("data/Caribbean_CTD.nc",'z_row_size');
    cur_idx = 1;
