...
% 读取坐标
T1 = readtable('附件.xlsx');
mirror_number = size(T1, 1);
% scatter(T1.x, T1.y)
% axis equal
...
for i = 1:mirror_number
    % 使用四象限反正切计算坐标夹角
    pos = ceil((atan2(T1.y(i), T1.x(i)) + pi) / sub_rad);
    sub_mirrors{pos} = [sub_mirrors{pos}; ...
        [T1.x(i), T1.y(i), param.height, param.len, param.width, sqrt(T1.x(i)^2 + T1.y(i)^2)]];
end
% 对立面点进行距离排序
for i = 1:subs
...
saveas(gcf, "q1年均效率.svg");
%% 第一问表一内容
load q1结果.mat
q1_sheet1 = q1_res(:, [1:4, 7]);
writematrix(q1_sheet1, 'q1表一.xlsx');

%% 第一问表二内容

load q1结果.mat
q1_res(:, 6) = q1_res(:, 6) / 1e3;
q1_sheet1 = q1_res(:, [1:4, 7]);
q1_sheet2 = mean(q1_res(:, [1:4, 6, 7]));
writematrix(q1_sheet1, 'q1表一.xlsx');
writematrix(q1_sheet2, 'q1表二.xlsx');
%% 函数
